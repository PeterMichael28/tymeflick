  ...
  return (
    <div className="w-full rounded-lg bg-white p-4">
      <div className="flex justify-between rounded-md bg-[#F3F3F3]">
        <div className="flex flex-col gap-5 px-4 py-2 w-[50%]">
          <p className="font-inter text-[24px] font-medium">{title}</p>
          {description && <p>{description}</p>}
          {isDashboard && (
            <span className="flex justify-between gap-4 rounded-lg border border-[#D2D9DF] bg-white px-3 py-2 w-[320px]">
        ...
        <img src="/image/Mask group (1).png" alt="Hero Image" className='w-[50%]' />
      </div>
    </div>
  )
}
