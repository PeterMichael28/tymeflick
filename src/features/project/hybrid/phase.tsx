...
const HybridPhase = () => {
  ...
  return (
    <div className="space-y-5">
      <div>
        <button
          className="bg-primary flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2 font-semibold text-white"
            ...
            <div className="flex w-[50vw] flex-col gap-4 rounded-lg bg-white p-4">
              <div className="mb-5 flex items-center justify-between">
                ...
                <X
                  ...
                  onClick={() => setOpenConfiguration(false)}
                />
              </div>
              {list.map((item, index) => (
                <div
                  className="flex cursor-pointer items-center justify-between space-y-4 bg-[#F7F6F9] p-3"
                  onClick={() => setSelectedPhase(index)}
                >
                  <div className="text-grey900 font-bricolage flex items-center gap-2 font-bold">
                    <p>{index + 1}</p>
                    <p>{item}</p>
                  </div>
                  <input
                    type="radio"
                    name="phase" // important: all radios must share the same name
                    value={index}
                    checked={selectedPhase === index}
                    onChange={() => setSelectedPhase(index)}
                    className="accent-primary border border-white"
                  />
                </div>
              ))}
            </div>
      ...
      <p className="font-bricolage text-lg font-bold">Phase Management</p>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="rounded-md border border-[#3370FF] bg-[#CCDBFF80] px-5 py-2 text-sm text-[#3370FF]">
                      4 Weeks
                    </p>
                  </div>
                ...
                <div>
                  {task.status === 'In Progress' && (
                    <Button className="mt-3 h-11 text-xs font-semibold">
