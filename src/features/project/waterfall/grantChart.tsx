  ...
  const renderMainTaskBar = (
    ...
    return (
      <div className="relative h-10 w-full">
        <div
          className={`absolute top-1/2 flex h-8 -translate-y-1/2 items-center justify-center rounded-full font-bold text-white ${allCompleted ? 'bg-[#0B54FF]' : 'bg-primary'} `}
                    ...
                    >
                      <label className="relative flex cursor-pointer items-center gap-2">
                        <input
                          type="checkbox"
                          checked={
                            completedTasks[`${grantIndex}-${i}`] || false
                          }
                          onChange={() => {
                            const key = `${grantIndex}-${i}`
                            setCompletedTasks((prev) => ({
                              ...prev,
                              [key]: !prev[key],
                            }))
                          }}
                          className="peer sr-only" // hides native checkbox
                        />
                        <div className="peer-checked:bg-primary peer-checked:border-primary flex h-4 w-4 items-center justify-center rounded-full border-2 border-gray-400">
                          <svg
        ...
        <div className="w-[240vw]">
          ...
          <div className="flex flex-col bg-white">
            {grantData.map((grant, grantIndex) => {
              ...
              const allCompleted = grant.task.every(
                (_, i) => completedTasks[`${grantIndex}-${i}`]
              )
