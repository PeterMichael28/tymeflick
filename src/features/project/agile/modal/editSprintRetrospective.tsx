 ...
 const [data, setData] = useState<Record<RetroSection, string[]>>({
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/20">
      <div className="flex flex-col gap-4 rounded-lg bg-white p-5 w-[90%] max-w-4xl">
        ...
        <div className="w-full flex flex-col mt-4">
          <Button className="h-13" onClick={handleClose}>Save Edit</Button>
          <button className="h-13 text-[#404C59] font-bold font-bricolage text-lg" onClick={handleClose}>
            Cancel
          </button>
        </div>
      </div>
...
const EditableList = ({
  ...
  return (
    <div className="flex flex-col items-start gap-2">
      ...
      <div className="rounded-lg border border-[#D2D9DF] p-4 w-full">
        <ul className="ml-3 list-disc text-xs text-[#4F5E6E] space-y-2">
          {data[section].map((item, index) => (
            <li
              key={index}
              contentEditable
              suppressContentEditableWarning
              className="outline-none cursor-text hover:bg-gray-100 px-1 rounded"
              onBlur={(e) =>
