...
const Task = ({ onNext, onPrev }: TaskProps) => {
  ...
  const addTask = () => {
    setTasks(prev => [...prev, { id: prev.length + 1 }])
  }
    ...
    <div className='bg-[#F9F9F9] p-5 rounded-3xl flex flex-col gap-4'>
      ...
      <div className='flex justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <img src="/icon/Group (7).svg" alt="Icon" />
          <p className='font-normal text-[#2B323B] text-[14px]'>Tasks</p>
        </div>
      ...
      {tasks.map((task, index) => (
        <div key={task.id} className='bg-white p-3 rounded-lg mb-4'>
